...
type clientTest struct {
	...
	// reference server.
	cert []byte
	// key, if not nil, contains either a *rsa.PrivateKey, ed25519.PrivateKey, *falcon512.PrivateKey or
...
func TestHandshakeClientFalcon512(t *testing.T) {
	cert, _ := X509KeyPair([]byte(clientFalcon512CertificatePEM), []byte(clientFalcon512KeyPEM))

	test := &clientTest{
		name: "Falcon512",
		cert: testFalcon512Certificate,
		key:  testFalcon512PrivateKey,
	}
	runClientTestTLS12(t, test)
	runClientTestTLS13(t, test)
