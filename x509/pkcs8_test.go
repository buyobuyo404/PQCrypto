...
func TestPKCS8(t *testing.T) {
	...
	for _, test := range tests {
		...
		fmt.Println("pk -> PKCS#8: ")
		reserialised, err := MarshalPKCS8PrivateKey(privKey)
		if err != nil {
			t.Errorf("%s: failed to marshal into PKCS#8: %s", test.name, err)
			continue
		}
		fmt.Println("PKCS#8 -> pk: ")
		key, err := ParsePKCS8PrivateKey(reserialised)
		if err != nil {
			t.Errorf("%s: failed to parse into PKCS#8: %s", test.name, err)
			continue
		}
		fmt.Println(key)
		...
		switch k := key.(type) {
		case *rsa.PrivateKey:
			fmt.Println("rsa pk: ", k.PublicKey)

		case *ecdsa.PrivateKey:
			fmt.Println("ecdsa pk: ", k.PublicKey)

		case ed25519.PrivateKey:
			fmt.Println("ed25519 pk: ", k.Public())
		}
